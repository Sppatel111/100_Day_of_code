...
class MovieRateForm(FlaskForm):
    rating = IntegerField('Your rating out of 10. eg. 7.5',[NumberRange(1,10)])
...
def find_movie():
    ...
    if movie:
        ...
        try:
            ...
            db.session.commit()
            flash("added movie !!", "added")
            return redirect(url_for("home", id=new_movie.id))
        except IntegrityError:
            db.session.rollback()
            flash("This movie already exists in the database.", "warning")
            return redirect(url_for("add"))

# if not (db.session.query(db.select(Movie)).exists(new_movie['title'])):
# if db.session.query(Movie).filter_by(new_movie["title"] not in new_movie):
...
def delete():
    ...
    db.session.commit()
    flash("deleted movie successfully!!","delete")
    return redirect(url_for('home'))
